...
ASTUBaseCharacter::ASTUBaseCharacter(const FObjectInitializer& ObjInit) : Super(ObjInit.SetDefaultSubobjectClass<USTUCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
	...
	SpringArmComponent->SocketOffset = FVector(0.0f, 75.0f, 80.0f);
...
{
	const auto MeterialInst = GetMesh()->CreateAndSetMaterialInstanceDynamic(0);
	if(!MeterialInst) return;

	MeterialInst->SetVectorParameterValue(MaterialColorName, Color);
}
